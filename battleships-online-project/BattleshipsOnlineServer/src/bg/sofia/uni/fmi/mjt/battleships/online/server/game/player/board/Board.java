...
import static bg.sofia.uni.fmi.mjt.battleships.online.server.game.player.board.BoardConstants.INVALID_COORDINATES;
...
import static bg.sofia.uni.fmi.mjt.battleships.online.server.game.player.board.BoardConstants.SIMPLE_ATTACK_ARGS_COUNT;
...
public class Board {
    ...
    public PlaceResponseDTO placeShip(String raw) {
        ...
        if (tokens.length < SIMPLE_ATTACK_ARGS_COUNT) {
            return new PlaceResponseDTO(String.format(MISSING_ARGUMENT, BLUE, DEFAULT));
        }

        for (int i = 0; i < SIMPLE_ATTACK_ARGS_COUNT; i++) {
        ...
        try {
            ...
            return new PlaceResponseDTO(INVALID_COORDINATES);
        }
